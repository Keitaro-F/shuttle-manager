...
| 現在地 | フェーズ6完了、フェーズ7着手前 |
| 次の作業 | フェーズ7：結合テスト・本番デプロイ（今回は未着手） |
...
| 7 | 結合テスト・本番デプロイ | 未着手（次フェーズ） | 3〜6時間 |
...
[完了] フェーズ6：LINE返信・残量確認・送信取消
   ↓
[次]   フェーズ7：結合テスト・本番デプロイ（未着手）
```

## 8. 次のCodexタスクに渡す指示

フェーズ7の新しいタスクでは、次のプロンプトを使用する。

```text
/Users/user/dev/shuttle-manager を対象に、LINE連携のフェーズ7だけを進めてください。
...
- 本番用PostgreSQLの準備
- デプロイ先への環境変数設定
- `prisma migrate deploy` を実行できる構成
- Next.jsアプリのデプロイ
- LINE Developers ConsoleへのWebhook URL設定
- Webhook利用とWebhook再送の有効化
- 実グループでの報告・残量確認・取消・再送テスト
- 秘密情報がログへ出ていないことの確認
- 既存Web機能の回帰テスト
- READMEへのセットアップ・運用方法の記載

フェーズ6までの実装を前提にしてください。
フェーズ8以降には着手しないでください。

秘密情報はGit管理外に保存し、コードやMarkdownへ記録しないでください。
