...
| 現在地 | フェーズ5完了、フェーズ6着手前 |
| 次の作業 | フェーズ6：LINE返信・残量確認・送信取消（今回は未着手） |
...
| 6 | LINE返信・残量確認・送信取消 | 未着手（次フェーズ） | 3〜5時間 |
...
フェーズ6の新しいタスクでは、次のプロンプトを使用する。

```text
/Users/user/dev/shuttle-manager を対象に、LINE連携のフェーズ6だけを進めてください。
...
- 一般会話の無視
- `シャトル残量` コマンド
- `unsend` イベントによるReport削除
- 返信失敗時の安全なエラーログ
- 返信・残量確認・送信取消の自動テスト

フェーズ5で実装したWebhook受信処理を前提にしてください。
本番デプロイにはまだ着手しないでください。
