...
| 現在地 | フェーズ3完了、フェーズ4着手前 |
| 次の作業 | フェーズ4：LINE公式アカウント・Webhook準備（今回は未着手） |
...
| 4 | LINE公式アカウント・Webhook準備 | 未着手（次フェーズ） | 1〜3時間 |
...
[完了] フェーズ3：メッセージ解析・Report保存処理
   ↓
[次]   フェーズ4：LINE公式アカウント・Webhook準備（未着手）
   ↓
...
フェーズ4の新しいタスクでは、次のプロンプトを使用する。

```text
/Users/user/dev/shuttle-manager を対象に、LINE連携のフェーズ4だけを進めてください。
...
- LINE公式アカウントの作成
- Messaging APIの有効化
- Channel secretとChannel access tokenの取得
- Botのグループ参加、自動応答、Webhook再送の設定
- 必要な環境変数のローカル設定

フェーズ3で作成したメッセージ解析とReport保存サービスを前提にしてください。
Webhook受信コード、署名検証、LINE返信、送信取消にはまだ着手しないでください。
