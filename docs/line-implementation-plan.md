...
| 現在地 | フェーズ4完了、フェーズ5着手前 |
| 次の作業 | フェーズ5：Webhook受信・署名・グループ制限（今回は未着手） |
...
| 5 | Webhook受信・署名・グループ制限 | 未着手（次フェーズ） | 4〜7時間 |
...
[完了] フェーズ4：LINE公式アカウント・Webhook準備
   ↓
[次]   フェーズ5：Webhook受信・署名・グループ制限（未着手）
   ↓
...
フェーズ5の新しいタスクでは、次のプロンプトを使用する。

```text
/Users/user/dev/shuttle-manager を対象に、LINE連携のフェーズ5だけを進めてください。
...
- 未加工本文を使った署名検証
- 許可グループ以外のイベント除外
- webhookEventIdによる二重登録防止
- ReportとWebhookReceiptのtransaction保存
- Webhook処理の自動テスト

フェーズ4で設定したLINE公式アカウントとローカル環境変数を前提にしてください。
LINE返信、残量確認、送信取消、本番デプロイにはまだ着手しないでください。
